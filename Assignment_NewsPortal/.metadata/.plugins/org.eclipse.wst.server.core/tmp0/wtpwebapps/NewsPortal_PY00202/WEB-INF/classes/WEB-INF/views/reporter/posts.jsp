    ...
    <div class="content-container">
        ...
        <div class="page-header">
            <div class="page-title-section">
                ...
                <p class="page-subtitle">Xem và quản lý tất cả bài viết của bạn</p>
            </div>
                ...
                <tr>
                  ...
                  <td class="post-title">
                    <div class="title-wrapper">
                      ...
                      <c:if test="${post.home}">
                        <span class="badge home">Trang nhất</span>
                      </c:if>
                    </div>
                  </td>
                  <td>
                  ...
                  <td class="actions">
                    <div class="action-buttons">
                      <a href="${pageContext.request.contextPath}/news/detail?id=${post.id}" 
                         class="btn-small ghost" title="Xem bài viết">👁️</a>
                      <a href="${pageContext.request.contextPath}/reporter/post-edit?id=${post.id}" 
                         class="btn-small" title="Chỉnh sửa">✏️</a>
                      <form method="post" action="${pageContext.request.contextPath}/reporter/post-delete" 
                        ...
                        <button type="submit" class="btn-small danger" title="Xóa bài viết">🗑️</button>
                      </form>
                    </div>
                  </td>
                </tr>
            ...
            <div class="mobile-card">
              ...
              <div class="mobile-card-meta">
                <div class="mobile-card-meta-item">
                  <span class="mobile-card-meta-label">Chuyên mục</span>
                  <span class="mobile-card-meta-value">
                    ${categoryMap[post.categoryId] != null ? categoryMap[post.categoryId] : 'Chưa phân loại'}
                  </span>
                </div>
                <div class="mobile-card-meta-item">
                  <span class="mobile-card-meta-label">Lượt xem</span>
                  <span class="mobile-card-meta-value">${post.viewCount}</span>
                </div>
                <div class="mobile-card-meta-item">
                  <span class="mobile-card-meta-label">Ngày đăng</span>
                  <span class="mobile-card-meta-value">
                    <c:choose>
                      <c:when test="${post.postedDate != null}">
                ...
                <div class="mobile-card-meta-item">
                  <span class="mobile-card-meta-label">Thời gian</span>
                  <span class="mobile-card-meta-value">
                    <c:choose>
                      <c:when test="${post.postedDate != null}">
              ...
              <div class="mobile-card-actions">
                <a href="${pageContext.request.contextPath}/news/detail?id=${post.id}" 
                   class="btn-small ghost">👁️ Xem</a>
                <a href="${pageContext.request.contextPath}/reporter/post-edit?id=${post.id}" 
                   class="btn-small">✏️ Sửa</a>
                <form method="post" action="${pageContext.request.contextPath}/reporter/post-delete" 
                      style="display: inline;" onsubmit="return confirm('Bạn có chắc muốn xóa bài viết này?');">
                  <input type="hidden" name="id" value="${post.id}">
                  <button type="submit" class="btn-small danger">🗑️ Xóa</button>
                </form>
              </div>
            </div>
          </c:forEach>
...
.table th:nth-child(2), .table td:nth-child(2) { width: 30%; } /* Tiêu đề */
.table th:nth-child(3), .table td:nth-child(3) { width: 12%; } /* Chuyên mục */
.table th:nth-child(4), .table td:nth-child(4) { width: 15%; } /* Ngày đăng */
.table th:nth-child(5), .table td:nth-child(5) { width: 8%; }  /* Lượt xem */
.table th:nth-child(6), .table td:nth-child(6) { width: 12%; } /* Trạng thái */
.table th:nth-child(7), .table td:nth-child(7) { width: 17%; } /* Thao tác */
...
@media (min-width: 769px) and (max-width: 1024px) {
    ...
    .table th:nth-child(1), .table td:nth-child(1) { width: 30%; } /* Tiêu đề */
    .table th:nth-child(2), .table td:nth-child(2) { width: 15%; } /* Chuyên mục */
    .table th:nth-child(3), .table td:nth-child(3) { width: 12%; } /* Ngày đăng */
    .table th:nth-child(4), .table td:nth-child(4) { width: 8%; }  /* Lượt xem */
    .table th:nth-child(5), .table td:nth-child(5) { width: 15%; } /* Trạng thái */
    .table th:nth-child(6), .table td:nth-child(6) { width: 20%; } /* Thao tác */
}
...
/* ===== Actions column: center + equal-sized buttons ===== */

/* Căn giữa toàn bộ cột Thao tác (cột 7) và đặt bề rộng hợp lý */
.table th:nth-child(7),
.table td:nth-child(7) {
  text-align: center;
  width: 160px;     /* có 3 nút -> 150–180px tùy bạn */
  min-width: 150px;
}

/* Hàng nút trong cột Thao tác: luôn căn giữa ngang */
.table td.actions,
.action-buttons {
  display: flex;
  justify-content: center !important;
  align-items: center;
  gap: 0.35rem;           /* thu nhỏ khoảng cách */
  flex-wrap: nowrap;      /* tránh xuống dòng khi đủ chỗ */
}

/* Nút vuông, nhỏ, bằng nhau – áp dụng cho cả link và button */
.action-buttons a.btn-small,
.action-buttons button.btn-small,
.action-buttons .btn-small[type="submit"] {
  width: 32px;            /* kích thước nút */
  height: 32px;
  min-width: 32px;        /* bỏ min-width 40px mặc định */
  padding: 0 !important;  /* emoji/icon tự căn giữa */
  border-radius: 6px;
  display: inline-flex;
  align-items: center;
  justify-content: center;
  font-size: 16px;        /* size emoji/icon */
  line-height: 1;
  text-align: center;
}

/* Trạng thái màu giữ nguyên theo loại nút hiện có */
.action-buttons a.btn-small.ghost {
  background: #f1f5f9;
  color: #64748b;
  border: 1px solid #e2e8f0;
}
.action-buttons a.btn-small:not(.ghost):not(.danger),
.action-buttons button.btn-small:not(.ghost):not(.danger),
.action-buttons .btn-small[type="submit"]:not(.ghost):not(.danger) {
  background: linear-gradient(135deg, #3b82f6, #2563eb);
  color: #fff;
  border: none;
}
.action-buttons .btn-small.danger,
.action-buttons .btn-small[type="submit"].danger {
  background: linear-gradient(135deg, #ef4444, #dc2626);
  color: #fff;
}

/* Giảm hiệu ứng hover để nút nhỏ không bị “nhảy” nhiều */
.action-buttons .btn-small:hover {
  transform: translateY(-1px);
}

/* Trường hợp icon là emoji/text – cố định chiều cao để không rung */
.action-buttons .btn-small > * {
  line-height: 1;
}

/* Responsive: co lại một chút ở tablet/mobile lớn */
...
/* (Mobile < 480px dùng layout thẻ card, không ảnh hưởng bảng) */
