...
public class MyMemoryTranslateService implements TranslationService {
  ...
  private String translatePlain(String src) throws Exception {
    ...
    for (String c : chunks) out.append(translateChunk(c));
    return out.toString();
  }
    ...
    NodeTraversor.traverse(new NodeVisitor() {
      @Override public void head(Node node, int depth) {
        try {
          ...
          // Không ném lỗi để khỏi mất toàn bộ bài; có thể log nếu bạn có logger
        }
      }
  ...
  private String translateChunk(String c) throws Exception {
    HttpUrl url = BASE.newBuilder()
        .addQueryParameter("q", c)          // để OkHttp tự encode
        .addQueryParameter("langpair", "vi|en")
        .build();
